...
- **Demo HTML pages** (pictured above) for each version so you can preview the output

This gives you a modern, minimal, and scalable icon system that can be used in any HTML or CSS setup — no JS required. This will be a good icon solution for you if:

- You have a directory full of custom SVG files you’d like to use as icons.
...
Before deploying, verify that all icons display correctly in your target environment and check for any CSS class name conflicts. The CSS generated by `svgs-to-icons` uses wildcard selectors like `[class*="-icon"]` to apply base styles, which can potentially affect existing classes in your project that contain the same patterns, i.e., 

If you encounter conflicts, use the `--prefix` and `--postfix` options to create more distinctive class names (e.g., `--prefix "svg-"` or `--postfix "-svg"`). See the CLI Options for 
...
**Note:** The `--bind 127.0.0.1` flag is required for the copy functionality in the demo pages to work properly in modern browsers.

### 3. I can’t see the icons. What should I do?
...
- Make sure your serving the page using a webserver, as mentioned in the [preceding FAQ](#why-do-i-need-a-server-for-file-referenced-icons).
...
The icons are fully **colorizable** using `color` on the icon or a parent element. These icons **do not support** multiple colors within the same shape. They’re single-shape, single-color icons — just like the icons rendered by icon fonts.
...
`svgs-to-icons` supports two common ways of displaying icons with CSS: embedded data URIs and SVG file references. These are compatible with any HTML code (including that generated by framworks) and do not require Javascript. Most framework components inject SVG markup directly into the HTML instead, either at runtime (using Javascript) or via a build process. Component libraries are certainly convenient, especially when built into UI frameworks like shadcn, Vuetify, Material Design, etc., but it may be more difficult to use custom icons with these frameworks.
...
For contributors and developers wanting to work with the source code:
...
- CSS class name generation and collision handling
- File system operations and output structure
