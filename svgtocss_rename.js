	...
	files.forEach((file) => {
		...
		if (fileExt === ".svg") {
			try {
				...
				const newFilePath = `${folderName}/lt5-${stringToClassName(file)}.svg`;
				try {
					const newFilePath = `${folderName}/lt5-${stringToClassName(file)}.svg`;
					...
					switch (error.code) {
						case 'ENOENT':
							console.error('Source file does not exist:', error.message);
							break;
						case 'EACCES':
						...
						case 'EPERM':
							...
							break;
						case 'EXDEV':
							console.error('Cross-device link error:', error.message);
							break;
						default:
